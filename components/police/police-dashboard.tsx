...
import { LogOut, Plus, Clock, AlertTriangle, Bot, TrendingUp, FileText } from "lucide-react"
...
export function PoliceDashboard({ user, onLogout }: PoliceDashboardProps) {
  ...
  const handleNewFIR = () => { setActiveTab("firs"); setFirView("form") }
  const handleFIRSuccess = () => setFirView("list")
  const handleViewFIR = (fir: FIR) => { setSelectedFIR(fir); setFirView("details") }
  const handleEditFIR = (fir: FIR) => { setSelectedFIR(fir); setFirView("form") }
  const handleBackToList = () => { setSelectedFIR(null); setFirView("list") }
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <main className="p-6 space-y-6">
        ...
        {activeTab === "firs" && (
          <>
            {firView==="list" && <FIRList onViewFIR={handleViewFIR} onEditFIR={handleEditFIR} />}
            {firView==="form" && <FIRForm onSuccess={handleFIRSuccess} onCancel={handleBackToList} editingFIR={selectedFIR} />}
            {firView==="details" && selectedFIR && <FIRDetails fir={selectedFIR} onBack={handleBackToList} onEdit={()=>handleEditFIR(selectedFIR)} />}
          </>
        )}
